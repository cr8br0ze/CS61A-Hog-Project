...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if opponent_score == 0:
        ...
        return roll_dice(num_rolls, dice) #+ tail_points(opponent_score)
...
def next_perfect_square(totalscore):
    this = 0
    for i in range(totalscore + 1):
        if totalscore == i * i:
            this = i
    that = this + 1
    nextsquare = that * that
    return nextsquare
# END PROBLEM 4
...
def play(strategy0, strategy1, update,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 = update(roll0, score0, score1, dice)
        else:
            roll1 = strategy1(score1, score0)
            score1 = update(roll1, score1, score0, dice)
        who = 1 - who
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for n in range(goal):
        for m in range(goal):
            if strategy(n, m) != strategy(n, m + 1):
                return False
    return True
...
def make_averaged(original_function, total_samples=1000):
    ...
    def func(*agr):
        ...
        for i in range(total_samples):
            total = total + original_function(*agr)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    count = 1
    i = 2
    max = make_averaged(roll_dice, total_samples)(1, dice)
    while i <= 10:
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    # END PROBLEM 11
    new = square_update(0, score, opponent_score)
    difference = new - score
